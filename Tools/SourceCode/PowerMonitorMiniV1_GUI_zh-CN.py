...
class PowerMonitorMiniV1_GUI:
    def __init__(self):
        ...
        self.btncom_reflash = ttk.Button(
            self.frame_com, text="刷新", command=self.btn_com_reflash
        )

        # 端口号
        self.labelport = tk.Label(self.frame_com, text="端口号：")
        ...
        # 波特率
        self.labelbps = tk.Label(self.frame_com, text="波特率:")
        ...
        self.com_with_crc_checkbutton = ttk.Checkbutton(
            self.frame_com, text="使能 CRC", variable=self.com_with_crc_check
        )
        ...
        self.label_device_info = tk.Label(
            self.frame_device_info, text="设备名称：", anchor=tk.W
        )
        ...
        self.label_device_sn = tk.Label(
            self.frame_device_info, text="序列号：", anchor=tk.W
        )
        ...
        self.label_device_soft_version = tk.Label(
            self.frame_device_info, text="软件版本：", anchor=tk.W
        )
        ...
        self.label_device_rshunt = tk.Label(
            self.frame_device_info, text="采样电阻：", anchor=tk.W
        )
        ...
        self.label_device_input_type = tk.Label(
            self.frame_device_info, text="输入类型：", anchor=tk.W
        )
        ...
        self.label_output_data = tk.Label(
            self.frame_output_data,
            text="输出数据",
            font=("Microsoft YaHei", 14, "bold"),
        )
        self.label_output_data.grid(row=0, column=0, sticky=tk.W)
        self.label_output_voltage = tk.Label(self.frame_output_data, text="电压")
        ...
        self.label_output_current = tk.Label(self.frame_output_data, text="电流")
        ...
        self.label_output_power = tk.Label(self.frame_output_data, text="功率")
        ...
        self.label_output_data_max_voltage = tk.Label(
            self.frame_output_data_max, text="最大电压", anchor=tk.W
        )
        ...
        self.label_output_data_max_current = tk.Label(
            self.frame_output_data_max, text="最大电流", anchor=tk.W
        )
        ...
        self.label_output_data_max_power = tk.Label(
            self.frame_output_data_max, text="最大功率", anchor=tk.W
        )
        ...
        self.label_pd = tk.Label(
            self.frame_pd, text="PD协议配置", font=("Microsoft YaHei", 14, "bold")
        )
        ...
        self.tree_pd.heading("Type", text="类型")
        self.tree_pd.heading("Max Voltage", text="最大电压(mV)")
        self.tree_pd.heading("Min Voltage", text="最小电压(mV)")
        self.tree_pd.heading("Max Current", text="最大电流(mA)")
        self.tree_pd.heading("Max Power", text="最大功率(W)")
        ...
        self.btn_ina226_reflash = ttk.Button(
            self.frame_ina226_config, text="读取", command=self.btn_ina226_reflash
        )
        self.btn_ina226_reflash.grid(row=3, column=2, padx=1, pady=1)
        self.btn_ina226_apply = ttk.Button(
            self.frame_ina226_config, text="应用", command=self.btn_ina226_apply
        )
        ...
        self.label_ina226_current_offset0_display = tk.Label(
            self.frame_ina226_config, text="offset0显示值", anchor=tk.W
        )
        ...
        self.label_ina226_current_offset0_actual = tk.Label(
            self.frame_ina226_config, text="offset0实际值", anchor=tk.W
        )
        ...
        self.label_ina226_current_offset1_display = tk.Label(
            self.frame_ina226_config, text="offset1显示值", anchor=tk.W
        )
        ...
        self.label_ina226_current_offset1_actual = tk.Label(
            self.frame_ina226_config, text="offset1实际值", anchor=tk.W
        )
        ...
        self.label_ina226_current_offset_zero = tk.Label(
            self.frame_ina226_config, text="零点", anchor=tk.W
        )
        ...
        if self.com_task_step == 0:
            ...
            while True:
                try:
                    ...
                    if data[0] == "error":
                        messagebox.showerror("错误", data[1])
    ...
    def run_device_task(self):
        try:
            ...
            while input_type == "Initialization":
                ...
                retry_count += 1
                if retry_count > 2:
                    messagebox.showerror("错误", "输入类型获取失败")
                    self.thread_status = 0
                    self.device.close()
                    return
